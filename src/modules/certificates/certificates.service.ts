...
export class CertificatesService {
  ...
  private createPdf(p: Participant, ev: Event & { course: Course | null }): Promise<Buffer> {
    return new Promise(resolve => {
      ...
      grad.stop(0.0, '#BBC6E6');
      grad.stop(0.22, '#B8C1E0');
      grad.stop(0.5, '#b5bedd');
      grad.stop(0.78, '#aeb9d7');
      grad.stop(1.0, '#a2b1d0');
      ...
      const dataEvento = new Intl.DateTimeFormat('pt-BR', {
        day: '2-digit',
        month: 'long',
        year: 'numeric',
        timeZone: 'UTC',
      ...
      const curso = ev.course?.name ?? 'Todos os cursos';
      const semestre = ev.semester && ev.semester !== 'ALL' ? ` • Semestre: ${ev.semester}` : '';
      ...
      doc.font('Helvetica').fontSize(18).text(texto, 60, contentTop, {
        width: width - 120,
